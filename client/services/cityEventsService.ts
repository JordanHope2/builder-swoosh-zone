...
export interface CityEvent {
  ...
  description: string;
  venue?: string;
  category: 'cultural' | 'sports' | 'business' | 'music' | 'arts' | 'festival' | 'other';
  url?: string;
}
...
class CityEventsService {
  ...
  private async getCachedCityData(cityName: string): Promise<CityEventsData | null> {
    try {
  ...
  private async generateCityOverviewWithAI(cityName: string): Promise<CityOverview> {
    try {
      const prompt = `
      ...
      const response = await fetch('https://api.openai.com/v1/chat/completions', {
        ...
        body: JSON.stringify({
          ...
          messages: [
            {
              role: 'system',
              content: 'You are a travel and city information expert. Provide accurate, concise information about cities.'
            },
            {
              role: 'user',
              content: prompt
  ...
  private getFallbackCityOverview(cityName: string): CityOverview {
    ...
    const cityData: Record<string, CityOverview> = {
      zurich: {
        ...
        description: 'Switzerland\'s largest city and economic powerhouse, Zurich combines world-class business opportunities with exceptional quality of life.',
      ...
      geneva: {
        ...
        description: 'An international city hosting numerous global organizations, Geneva offers a cosmopolitan lifestyle with stunning Alpine views.',
      ...
      basel: {
        ...
        description: 'A cultural and pharmaceutical powerhouse on the Rhine, Basel blends innovation with rich artistic heritage.',
      ...
      bern: {
        ...
        description: 'Switzerland\'s charming capital city, featuring a UNESCO-listed medieval old town and serving as the political center.',
      ...
      lausanne: {
        ...
        description: 'The Olympic capital situated on Lake Geneva, Lausanne combines academic excellence with sports heritage and scenic beauty.',
  ...
  private generateMockEvents(cityName: string): CityEvent[] {
    ...
    const eventTemplates = [
      ...
      },
      {
        ...
        description: 'Premier international art fair showcasing contemporary and modern art',
        venue: 'Messe Basel'
      },
      {
        title: `${cityName} Marathon`,
        category: 'sports' as const,
        description: 'Annual international marathon through the city\'s most scenic routes',
        venue: 'City Center'
      },
      {
        title: 'Swiss National Day Celebration',
        category: 'cultural' as const,
        description: 'Traditional Swiss national day festivities with fireworks and local culture',
        venue: 'City Square'
      }
    ];
    ...
    for (let i = 0; i < Math.min(5, eventTemplates.length); i++) {
      ...
      const eventDate = new Date(now.getTime() + (i + 1) * 7 * 24 * 60 * 60 * 1000); // 1-5 weeks from now
  ...
  private generateQuickLinks(cityName: string): { tourism: string; events: string; official: string } {
