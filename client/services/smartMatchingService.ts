...
class SmartMatchingService {
  ...
  async findJobMatches(candidateId: string, limit: number = 10): Promise<JobMatch[]> {
    try {
  ...
  private async calculateMatches(profile: CandidateProfile, jobs: Job[]): Promise<JobMatch[]> {
  ...
  private async calculateSingleMatch(profile: CandidateProfile, job: Job): Promise<JobMatch> {
    // Skill matching
    const skillMatches = this.calculateSkillMatches(profile.skills, job.skills, job.requirements);
    const skillScore = this.calculateSkillScore(skillMatches);

    // Location matching
    const locationMatch = this.calculateLocationMatch(profile, job);
    const locationScore = locationMatch ? 20 : (job.remote && profile.remote ? 15 : 0);
    ...
    const matchScore = Math.min(100, skillScore + locationScore + salaryScore + experienceScore);
  ...
  private calculateSkillMatches(candidateSkills: string[], jobSkills: string[], requirements: string[]): SkillMatch[] {
    ...
    for (const jobSkill of allJobSkills) {
      ...
      if (normalizedCandidateSkills.includes(normalizedJobSkill)) {
        ...
        const similarSkills = this.findSimilarSkills(normalizedJobSkill, normalizedCandidateSkills);
      ...
      skillMatches.push({
        ...
        required: isRequired,
        candidateHas,
        experience: candidateHas ? this.estimateSkillExperience(jobSkill, candidateSkills) : 0,
        match
      });
    }
  ...
  private findSimilarSkills(targetSkill: string, candidateSkills: string[]): string[] {
  ...
  private estimateSkillExperience(skill: string, candidateSkills: string[]): number {
  ...
  private calculateLocationMatch(profile: CandidateProfile, job: Job): boolean {
    ...
    return normalizeLocation(profile.location).includes(normalizeLocation(job.location)) ||
  ...
  private calculateSalaryMatch(profile: CandidateProfile, job: Job): boolean {
    ...
    const profileSalaryAvg = (profile.salaryExpectation.min + profile.salaryExpectation.max) / 2;

    // Consider it a match if within 20% range
    const tolerance = 0.2;
    return Math.abs(jobSalaryAvg - profileSalaryAvg) / profileSalaryAvg <= tolerance;
  }

  private calculateExperienceMatch(profile: CandidateProfile, job: Job): boolean {
    ...
    const requiredExp = experienceMapping[job.experience_level?.toLowerCase()] || 3;
  ...
  private generateMatchReasons(
    ...
    // Skill reasons
    const exactSkills = skillMatches.filter(s => s.match === 'exact' && s.candidateHas).length;
    const similarSkills = skillMatches.filter(s => s.match === 'similar' && s.candidateHas).length;
    
    if (exactSkills > 0) {
      reasons.push(`${exactSkills} exact skill match${exactSkills > 1 ? 'es' : ''}`);
    }
    if (similarSkills > 0) {
      reasons.push(`${similarSkills} related skill${similarSkills > 1 ? 's' : ''}`);
    }
    ...
    return [
      {
        ...
        candidateId,
        matchScore: 92,
        matchReasons: ['8 exact skill matches', 'Location match', 'Salary range alignment'],
        skillMatches: [],
        locationMatch: true,
        salaryMatch: true,
        ...
        job: {
          ...
          salary_min: 120000,
          salary_max: 140000,
          description: 'Join our innovative team building next-generation technology solutions.',
