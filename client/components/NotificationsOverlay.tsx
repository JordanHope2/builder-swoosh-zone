...
interface Notification {
  id: string;
  type: 'job_match' | 'application' | 'message' | 'system' | 'achievement' | 'job_alert';
  title: string;
  message: string;
  timestamp: Date;
  read: boolean;
...
const mockNotifications: Notification[] = [
  {
    ...
    message: 'Senior Software Engineer at TechCorp (95% match) - Perfect for your skills in React and TypeScript',
  ...
  },
  {
    ...
    message: 'Your application for Product Manager at InnovateCH has been reviewed',
  ...
  },
  {
    ...
    type: 'message',
    title: 'New Message',
    message: 'Sarah from TechCorp sent you a message about the Senior Engineer position',
...
export function NotificationsOverlay() {
  ...
  const getNotificationIcon = (type: string, priority: string) => {
    const iconClass = priority === 'high' ? 'w-5 h-5 text-red-500' : 
                     priority === 'medium' ? 'w-5 h-5 text-yellow-500' : 
                     'w-5 h-5 text-blue-500';

    switch (type) {
      case 'job_match': return <Briefcase className={iconClass} />;
      ...
      case 'job_alert': return <Bell className={iconClass} />;
  ...
  const filteredNotifications = notifications.filter(n => {
    ...
    if (activeTab === 'jobs') return n.type === 'job_match' || n.type === 'job_alert';
    return true;
  });
              ...
              <div className="p-6 border-b border-gray-200 dark:border-gray-700">
                ...
                <div className="flex space-x-1 bg-gray-100 dark:bg-gray-700 rounded-lg p-1">
                  {[
                    ...
                    { id: 'jobs', label: 'Jobs', count: notifications.filter(n => n.type === 'job_match' || n.type === 'job_alert').length }
                  ].map((tab) => (
                    <button
                      key={tab.id}
                            ...
                            {notification.actionUrl && (
                              <div className="mt-3">
                                <a
                                  ...
                                  onClick={() => markAsRead(notification.id)}
                                >
                                  {notification.type === 'job_match' ? 'View Job' :
