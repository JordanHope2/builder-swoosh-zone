...
export function SwipeJobDiscovery() {
  ...
  if (!currentJob && swipedJobs.length === 0) {
    return (
      <div className="text-center p-8">
        ...
        <h3 className="text-xl font-semibold text-jobequal-text mb-2">You're all caught up!</h3>
        <p className="text-jobequal-text-muted">Check back later for new job matches.</p>
      </div>
    );
  }
  ...
  if (!currentJob) {
    return (
      <div className="text-center p-8">
        <div className="text-6xl mb-4">✨</div>
        <h3 className="text-xl font-semibold text-jobequal-text mb-2">No more jobs to review!</h3>
    ...
    <div className="max-w-md mx-auto relative">
      ...
      <div className="mb-4">
        <div className="flex justify-between text-sm text-jobequal-text-muted mb-2">
          <span>Job {currentJobIndex + 1} of {mockSwipeJobs.length}</span>
          <span>{swipedJobs.filter(s => s.action === 'like').length} liked</span>
        </div>
        ...
        >
          ...
          <div className="p-6 bg-gradient-to-r from-jobequal-green/10 to-jobequal-teal/10">
            <div className="flex items-center justify-between mb-4">
              <div className={`flex items-center space-x-2 px-3 py-1 rounded-full text-sm font-bold ${getMatchColor(currentJob.matchScore)}`}>
          ...
          <div className="p-6 flex-1 overflow-y-auto">
            <div className="space-y-4">
              ...
              {/* Description */}
              <div className={`transition-all duration-300 ${showDetails ? '' : 'line-clamp-3'}`}>
              ...
              {showDetails && (
                ...
                >
                  {/* Why This Match */}
                  <div>
                    <h4 className="font-semibold text-jobequal-text dark:text-white mb-2">Why this match?</h4>
                    <div className="space-y-1">
                      {currentJob.whyMatch.map((reason, index) => (
                        <div key={index} className="flex items-center text-sm text-jobequal-text-muted dark:text-gray-300">
                  ...
                  {/* Requirements */}
                  <div>
                    <h4 className="font-semibold text-jobequal-text dark:text-white mb-2">Requirements</h4>
                    <div className="flex flex-wrap gap-2">
                      {currentJob.requirements.map((req, index) => (
                        <span key={index} className="bg-jobequal-green-light dark:bg-gray-700 text-jobequal-green-dark dark:text-gray-300 px-3 py-1 rounded-full text-sm font-medium">
                          {req}
                        </span>
                      ))}
                    </div>
                  ...
                  {/* Benefits */}
                  <div>
                    <h4 className="font-semibold text-jobequal-text dark:text-white mb-2">Benefits</h4>
                    <div className="flex flex-wrap gap-2">
