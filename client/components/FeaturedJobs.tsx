...
function JobCard({ job, index }: { job: Job; index: number }) {
  ...
  const logoPlaceholder = companyName
    ? companyName.charAt(0).toUpperCase()
    : "J";

  return (
    <div
      className={`bg-white/90 backdrop-blur-sm rounded-2xl sm:rounded-3xl p-4 sm:p-6 lg:p-8 border border-jobequal-neutral-dark hover:shadow-2xl transition-all duration-500 group cursor-pointer transform hover:scale-105 hover:bg-white relative overflow-hidden ${
    ...
    >
      ...
      <div className="relative z-10">
        <div className="text-center mb-4 sm:mb-6">
          <div
            className={`inline-flex items-center space-x-2 px-3 sm:px-4 py-2 rounded-xl sm:rounded-2xl text-sm sm:text-base lg:text-lg font-bold border-2 animate-pulse-soft ${getMatchColor(job.matchScore)}`}
          >
            <Zap className="w-4 h-4 sm:w-5 sm:h-5 animate-bounce-subtle" />
            <span>
              {job.matchScore}% {t("featured.match")}
            </span>
          </div>
        </div>
        <button
        ...
        >
          <Heart
            className={`w-4 h-4 sm:w-5 sm:h-5 transition-all duration-300 group-hover/heart:animate-pulse ${isLiked ? "fill-current scale-110" : ""}`}
          />
        </button>
        <div className="flex items-start space-x-3 sm:space-x-4 lg:space-x-5 mb-4 sm:mb-6">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-jobequal-text-muted font-medium text-sm sm:text-base lg:text-lg truncate">
              {companyName}
            </p>
          </div>
        </div>
...
export function FeaturedJobs() {
  ...
  return (
    <section
      id="featured-jobs"
      className="py-16 sm:py-20 lg:py-32 bg-gradient-to-b from-jobequal-neutral to-white relative overflow-hidden"
    >
      <div className="absolute top-20 left-10 w-32 h-32 bg-jobequal-green/5 rounded-full blur-3xl animate-pulse-soft" />
      <div
        className="absolute bottom-20 right-10 w-40 h-40 bg-jobequal-teal/5 rounded-full blur-3xl animate-pulse-soft"
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        >
          <div className="flex items-center justify-center mb-4 sm:mb-6">
            ...
            <Star
              className="w-5 h-5 sm:w-6 sm:h-6 text-jobequal-green ml-2 sm:ml-3 animate-bounce-subtle"
        ...
        <div
          className={`grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 sm:gap-8 lg:gap-10 mb-12 sm:mb-16 transition-all duration-1000 delay-300 ${isVisible ? "opacity-100" : "opacity-0"}`}
        >
          {isLoading ? (
            Array.from({ length: 6 }).map((_, index) => (
              <div
                key={index}
                className="bg-white/50 rounded-2xl p-8 h-96 animate-pulse"
              ></div>
            ))
          ) : error ? (
            <div className="col-span-full text-center text-red-500 bg-red-100 p-8 rounded-2xl">
              {error}
            </div>
          ) : (
            jobs.map((job, index) => (
