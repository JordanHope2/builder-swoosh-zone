...
export default function SubscriptionBilling() {
  ...
  const currentPlan = pricingPlans.find(
    (plan) => plan.id === mockSubscription.planId,
  );
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-sm text-jobequal-text-muted dark:text-gray-400">
                  {currentPlan?.type === "company"
                    ? "Company Plan"
                    : currentPlan?.type === "recruiter"
                      ? "Recruiter Plan"
                      : "Candidate Plan"}
                </p>
              </div>
            ...
            <div className="flex items-center space-x-2">
              <span
                className={`px-3 py-1 rounded-full text-xs font-medium ${getStatusColor(mockSubscription.status)}`}
              >
      ...
      {["candidate", "recruiter", "company"].map((type) => (
        <div key={type} className="space-y-6">
          <SectionHeader
            ...
            subtitle={
              type === "candidate"
                ? "Perfect for job seekers looking to advance their careers"
                : type === "recruiter"
                  ? "Comprehensive solutions for talent acquisition professionals"
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-gray-200 dark:border-gray-700">
                <th className="text-left py-3 px-4 font-medium text-jobequal-text dark:text-white">
                  Invoice
                </th>
                <th className="text-left py-3 px-4 font-medium text-jobequal-text dark:text-white">
                  Date
          ...
          {mockPaymentMethods.map((method) => (
            <div
              ...
              className={`p-4 border rounded-lg ${
                method.isDefault
                  ? "border-jobequal-green bg-green-50 dark:bg-green-900/10"
    ...
    <div className="space-y-8">
      ...
      <DashboardCard title="Usage Analytics">
        <div className="space-y-6">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <div>
              <h4 className="font-medium text-jobequal-text dark:text-white mb-4">
                Plan Limits
              </h4>
              <div className="space-y-4">
                <div>
